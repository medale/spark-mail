...
object DataFrameFromRddDynamicSchema {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val types =
      List(("emailPrefix", StringType), ("name", StringType), ("position", StringType), ("location", StringType))

    val fields = types.map { case (name, structType) =>
    ...
    val rolesRowRdd =
      rolesRdd.map(s => s.split(",")).map(lineArray => Row(lineArray(0), lineArray(1), lineArray(2), lineArray(3)))
